...
import { useEditWorkerManagementByListIdMutation } from "../../../components/APIs/WorkerManagement/WORKER_MANAGEMENT_QUERY";
...
export default function GroupEditWorkerManagementForm({
  open,
  close,
  ids,
  onSuccess,
  ...
  const [editWorkerManagementByListId, { isLoading }] =
  ...
  const {
    ...
    defaultValues: {
      availableNoWorkers: undefined,
      remainNoOfWorkers: undefined,
      notes: "",
    },
  });

  // Start with empty fields every time the modal opens (no prefilled values).
  useEffect(() => {
    if (open) {
      reset({
        availableNoWorkers: undefined,
  ...
  const onSubmit = async (values: GroupEditFormValues) => {
    if (!ids.length) return;

    const payload: workerManagementGroupEditProps = {
      ids,
      availableNoWorkers: Number(values.availableNoWorkers) || 0,
      remainNoOfWorkers: Number(values.remainNoOfWorkers) || 0,
      notes: values.notes,
    };

    try {
      await editWorkerManagementByListId(payload).unwrap();
  ...
  return (
    <Modal
      title={`${t("GROUP_EDIT_WORKER_MANAGEMENT")} (${ids.length})`}
